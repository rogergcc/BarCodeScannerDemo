...
import com.rogergcc.barcodescannerdemo.databinding.QuickieOverlayViewBinding
...
class QROverlayView @JvmOverloads constructor(
    ...
    private fun Drawable.limitDrawableSize(): Drawable {
        ...
        if (scale < 1) {
            setBounds(
                0,
                0,
                (minimumWidth * scale).roundToInt(),
                (minimumHeight * scale).roundToInt()
            )
        } else {
    ...
    private fun View.setTintAndStateAwareBackground() {
        background?.let { drawable ->
            ...
            val colorStateList =
                ColorStateList(states, stateColors).withAlpha(BUTTON_BACKGROUND_ALPHA.roundToInt())
