...
import com.rogergcc.barcodescannerdemo.ui.camera.CameraSizePair
...
object PreferenceUtils {
  ...
  fun getProgressToMeetBarcodeSizeRequirement(overlay: GraphicOverlay, barcode: Barcode): Float {
    ...
    return if (getBooleanPref(context, R.string.pref_key_enable_barcode_size_check, true)) {
      ...
      val requiredWidth =
        reticleBoxWidth * getIntPref(context, R.string.pref_key_minimum_barcode_width, 40) / 100//
  ...
  fun getBarcodeReticleBox(overlay: GraphicOverlay): RectF {
    ...
    val boxHeight =
      overlayHeight * getIntPref(context, R.string.pref_key_barcode_reticle_height, 20) / 100//
